...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        VectorXd tmp_b(3);
        tmp_b.setZero();
		//相邻帧i,j间的位姿变换，由sfm的结果提供
        Eigen::Quaterniond q_ij(frame_i->second.R.transpose() * frame_j->second.R);
		//位姿（李代数表示）相对陀螺仪bias（3维向量）的雅克比矩阵3x3
        tmp_A = frame_j->second.pre_integration->jacobian.template block<3, 3>(O_R, O_BG);
		//通过imu预积分得到的帧i,j间的位姿变换与sfm提供的位姿变换之间的差residual_q
        tmp_b = 2 * (frame_j->second.pre_integration->delta_q.inverse() * q_ij).vec();
		//对(J_ij*delta_bg-residual_q)^T*((J_ij*delta_bg-residual_q)求和，然后求解最小值对应的delta_bg。
		//对delta_bg求导可得：
		//sum(J_ij^T*J_ij)*delta_bg=sum(J_ij^T*residual_q);
...
MatrixXd TangentBasis(Vector3d &g0)
{
	...
	//c和a,b都垂直，这样获得的b,c即是g0的在球面坐标上的一个切向方向，用于后续的重力方向的初始化
    c = a.cross(b);
    MatrixXd bc(3, 2);
    bc.block<3, 1>(0, 0) = b;
    bc.block<3, 1>(0, 1) = c;
