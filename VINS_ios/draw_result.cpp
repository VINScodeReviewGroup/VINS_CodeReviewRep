...
void DrawResult::drawArrowFllowedByPath(cv:: Mat &equ_image, cv::Mat &result, vector<Vector3f> &point_cloud, Vector3f P_latest, Matrix3f R_latest, bool vins_update, vector<Vector3f>& vinsPaths){
	cv::Mat aa(HEIGHT,WIDTH,CV_8UC3,Scalar(0,0,0));
	result = aa;
	Eigen::Matrix3f RIC;
	RIC = Utility::ypr2R(Vector3d(RIC_y,RIC_p,RIC_r)).cast<float>();
